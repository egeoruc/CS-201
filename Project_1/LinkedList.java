...
public class LinkedList {
    ...
    public LinkedList(int number) {
        if (number == 0) {
            // If the number is 0, create a linked list with a single node containing 0
            head = new Node(0);
        } else {
            while (number > 0) {
                int digit = number % 10; // Get the last digit of the number
                insertLast(new Node(digit)); // Insert the digit as a new node at the beginning
    ...
    public String toStringReverse() {
        ...
        toStringReverseHelper(head, result);
        return result.toString();
    }

    public void toStringReverseHelper(Node node, StringBuilder result) {
        if (node == null) {
            return;
        }
        toStringReverseHelper(node.getNext(), result);
